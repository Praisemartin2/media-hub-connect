...
import { COLORS, TITLE_CARDS, HOOK, CTA, BROLL, ASSETS_READY, SRC } from "./img2071-data";
import captions from "./img2071-captions.json";
  ...
  <AbsoluteFill>
    <OffthreadVideo
      ...
      style={{
        ...
        height: "100%",
        objectFit: "cover",
        filter: "contrast(1.06) saturate(1.05) brightness(1.03) sepia(0.06) hue-rotate(-6deg)",
      }}
    />
...
const BrandedBg: React.FC = () => {
  ...
  const hue = interpolate(frame, [0, 2400], [0, 18]);
  return (
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.ink }}>
      <OffthreadVideo
        src={staticFile(src)}
        muted
        style={{ width: "100%", height: "100%", objectFit: "cover", transform: `scale(${scale})`, filter: "contrast(1.05) saturate(1.05) sepia(0.05) hue-rotate(-5deg)" }}
      />
      <AbsoluteFill style={{ background: SCRIM }} />
    </AbsoluteFill>
  );
...
const BrollLayer: React.FC = () => {
  ...
  return (
    <>
      {BROLL.map((b, i) => (
        <Sequence key={i} from={Math.round(b.at * fps)} durationInFrames={Math.round(b.dur * fps)}>
          <BrollClip src={b.src} />
        </Sequence>
      ))}
    </>
  ...
  return (
    <AbsoluteFill style={{ justifyContent: "flex-end", alignItems: "flex-start", padding: 70, paddingBottom: 560, opacity: out }}>
      <div style={{ transform: `translateY(${y}px)`, opacity: inSpring }}>
        ...
        <div style={{ fontFamily: FONT, fontWeight: 900, fontSize: 110, lineHeight: 0.98, color: COLORS.white, letterSpacing: -2, textShadow: "0 6px 26px rgba(0,0,0,0.5)" }}>
...
export const IMG2071Reel: React.FC = () => {
  const { fps, durationInFrames } = useVideoConfig();
  const ctaFrom = Math.round(CTA.start * fps);
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.ink }}>
      {ASSETS_READY ? <Grade /> : <BrandedBg />}
