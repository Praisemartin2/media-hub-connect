...
import { COLORS, TITLE_CARDS, HOOK, CTA, BROLL, ASSETS_READY, SRC } from "./subjectto-data";
import captions from "./subjectto-captions.json";
...
// ---------- A-roll (warm grade) or branded fallback before footage arrives ----------
  ...
  <AbsoluteFill>
    <OffthreadVideo
      ...
      style={{
        ...
        height: "100%",
        objectFit: "cover",
        filter: "contrast(1.06) saturate(1.04) brightness(1.02) sepia(0.06) hue-rotate(-6deg)",
      }}
    />
...
const BrandedBg: React.FC = () => {
  ...
  return (
    <AbsoluteFill
      style={{ background: `linear-gradient(${135 + hue}deg, ${COLORS.espresso} 0%, ${COLORS.cobalt} 55%, ${COLORS.rust} 120%)` }}
    >
      <AbsoluteFill style={{ background: SCRIM }} />
    </AbsoluteFill>
  );
    ...
    <AbsoluteFill style={{ backgroundColor: COLORS.ink }}>
      <OffthreadVideo
        ...
        style={{
          ...
          filter: "contrast(1.05) saturate(1.05) sepia(0.05) hue-rotate(-5deg)",
        }}
      />
      <AbsoluteFill style={{ background: SCRIM }} />
    </AbsoluteFill>
...
export const SubjectToReel: React.FC = () => {
  const { fps, durationInFrames } = useVideoConfig();
