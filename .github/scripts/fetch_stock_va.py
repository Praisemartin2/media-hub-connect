...
TERMS = [
    ...
    "US dollar banknotes stack",
    "cash money dollars hand",
    "counting money dollars",
    "real estate handshake",
]

os.makedirs("stockva", exist_ok=True)
idx, seen = [], set()
for term in TERMS:
    url = ("https://commons.wikimedia.org/w/api.php?action=query&generator=search"
           f"&gsrsearch={urllib.parse.quote(term)}&gsrnamespace=6&gsrlimit=8"
...
for i, e in enumerate(idx[:32]):
    fn = f"stockva/cand_{i:02d}.jpg"
    try:
        ...
        e["file"] = None
json.dump(idx[:32], open("stockva/index.json", "w"), indent=1)
print("downloaded", sum(1 for e in idx[:32] if e.get("file")))
